...
const homeScreenIconButton = document.querySelector("#homeScreenIconButton");
const homeScreenModal = document.querySelector("#homeScreenModal");
const copyPhotoPageLinkButton = document.querySelector("#copyPhotoPageLinkButton");
const MAX_UPLOAD_FILES = 70;
const folderFromUrl = readFolderFromUrl();
...
function init() {
  ...
  homeScreenIconButton?.addEventListener("click", handleHomeScreenIconClick);
  copyPhotoPageLinkButton?.addEventListener("click", copyPhotoPageLink);
  homeScreenModal?.addEventListener("click", (event) => {
    if (event.target?.hasAttribute?.("data-home-screen-close")) {
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (event.key === "Escape" && homeScreenModal && !homeScreenModal.hidden) {
      closeHomeScreenModal();
    }
  });
  updateHomeScreenIconButton();
  if (folderFromUrl) {
    folderInput.value = folderFromUrl;
    setStatus(`Ready to upload to: ${folderFromUrl}`);
  }
  loadFolders(folderFromUrl);
}
...
function readFolderFromUrl() {
  ...
  return (params.get("folder") || params.get("fileFolder") || "").trim();
}
...
function registerServiceWorker() {
  ...
  navigator.serviceWorker.register("/sw.js").catch(() => {
    // The photo workflow still works if PWA registration is unavailable.
  });
}
...
function handleAppInstalled() {
  ...
  updateHomeScreenIconButton(true);
  setStatus("Photo Upload icon added to your home screen.");
}
...
function updateHomeScreenIconButton(installed = isStandaloneMode()) {
  if (!homeScreenIconButton) {
    return;
  }
  homeScreenIconButton.classList.toggle("is-installed", installed);
  const label = homeScreenIconButton.querySelector(".home-icon-label");
  if (label) {
    label.textContent = installed ? "Added" : "Add Icon";
  }
  homeScreenIconButton.setAttribute(
    "aria-label",
    installed ? "Photo Upload is already added" : "Add Photo Upload to home screen"
  );
}

async function handleHomeScreenIconClick() {
  if (isStandaloneMode()) {
    updateHomeScreenIconButton(true);
    setStatus("Photo Upload is already running from your home screen.");
    return;
  }
  if (deferredInstallPrompt && !isIosDevice()) {
    ...
    deferredInstallPrompt = null;
    updateHomeScreenIconButton(choice?.outcome === "accepted");
    if (choice?.outcome === "accepted") {
      setStatus("Photo Upload icon added to your home screen.");
    } else {
      setStatus("Home screen icon was not added.");
    }
    return;
  }
  openHomeScreenModal();
...
function openHomeScreenModal() {
  ...
  homeScreenModal.hidden = false;
  document.body.classList.add("home-screen-modal-open");
  copyPhotoPageLinkButton?.focus();
}
...
function closeHomeScreenModal() {
  ...
  homeScreenModal.hidden = true;
  document.body.classList.remove("home-screen-modal-open");
  homeScreenIconButton?.focus();
}

async function copyPhotoPageLink() {
  const url = window.location.href;
  try {
    await navigator.clipboard.writeText(url);
    setTemporaryButtonText(copyPhotoPageLinkButton, "Copied");
    setStatus("Photo Page link copied.");
  } catch (error) {
...
async function loadFolders(preferredFolder = "") {
  ...
  if (!data.folders.length) {
    ...
    if (preferredFolder) {
      folderInput.value = preferredFolder;
      photoGrid.innerHTML = `<p class="empty">No photos yet in ${escapeHtml(preferredFolder)}. Choose saved photos to upload.</p>`;
      setStatus(`Ready to create/upload to: ${preferredFolder}`);
    } else {
  ...
  if (preferredFolder) {
    const matchingOption = Array.from(folderSelect.options).some((option) => option.value === preferredFolder);
    if (matchingOption) {
